...
describe('The batsmans score', function() {
    it('should not increase on a dot ball', function() {
        var stats = getStats();
        var increment = { runs: 0, event: { ball: { innings: 1 }, batsman: { id: 1 }, eventType: 'delivery' } };
    ...
    it('should increase by 1 when a single is scored', function() {
        var stats = getStats();
        var increment = { runs: 1, event: { ball: { innings: 1 }, batsman: { id: 1 }, eventType: 'delivery' } };
        processor.incrementStats(stats, increment);
...
describe('The balls faced count', function() {
    it('should increment on a legal delivery', function() {
        var stats = getStats();
        var increment = { runs: 0, ballsFaced: 1, event: { ball: { innings: 1 }, batsman: { id: 1 }, eventType: 'delivery' } };
        processor.incrementStats(stats, increment);
        stats[1][1].ballsFaced.should.equal(1);
    });

    it('should not increment on a wide', function() {
        var stats = getStats();
        var increment = { runs: 0, ballsFaced: 0, event: { ball: { innings: 1 }, batsman: { id: 1 }, eventType: 'wide' } };
        processor.incrementStats(stats, increment);
        stats[1][1].ballsFaced.should.equal(0);
    });

    it('should increment on a no ball', function() {
        var stats = getStats();
        var increment = { runs: 0, ballsFaced: 1, event: { ball: { innings: 1 }, batsman: { id: 1 }, eventType: 'noBall' } };
        processor.incrementStats(stats, increment);
        stats[1][1].ballsFaced.should.equal(1);
    });
});
...
describe('The method of dismissal', function() {
    it('should be set when the batsman is dismissed', function() {
        var stats = getStats();
        var increment = { wicket: { eventType: 'bowled' }, event: { ball: { innings: 1 }, batsman: { id: 1 }, eventType: 'bowled' } };
        processor.incrementStats(stats, increment);
        stats[1][1].should.have.property('dismissal');
    });
    ...
    it('should not be set when the batsman is not dismissed', function() {
        ...
        var increment = { runs: 0, ballsFaced: 0, event: { ball: { innings: 1 }, batsman: { id: 1 }, eventType: 'wide' } };
        processor.incrementStats(stats, increment);
        should.not.exist(stats[1][1].dismissal);
    });
});
...
describe('The methods of scoring count', function() {
    ...
    it('should increment boundary four when there is a boundary', function() {
        var stats = getStats();
        var increment = { runs: 4, event: { ball: { innings: 1 }, batsman: { id: 1 }, eventType: 'delivery' } };
        processor.incrementStats(stats, increment);
